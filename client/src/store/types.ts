...
export type RoundResult = { scores: [number, number]; details?: RoundScoreDetails };
...
export type GameSlice = {
  ...
  setDisplayedTableCards: (cards: Card[]) => void;
...
export type UISlice = {
  ...
  opponentGhostSeat: PlayerIndex | null;
  /**
   * The card id of a card currently in flight from a hand to the table
   * (placement only). TableCards renders this id at opacity 0 so the user
   * doesn't see both the flight overlay and the static slot card at once.
   * Cleared by GameTable when all flights for the play finish.
   */
  flightInProgressId: string | null;
  ...
  setFlightInProgressId: (id: string | null) => void;
